...
describe("incident detail domain",()=>{
 ...
 it("shows API decisions and closure only to an authorized controller or manager",()=>{const reviewing={...incident,status:"UNDER_REVIEW"} as DashboardIncident;const decision=resolvePrimaryIncidentAction(reviewing,{public_id:"me",permissions:["INCIDENT.DECIDE"]});expect(decision?.key).toBe("decide");expect(decision&&isIncidentActionSupported("api",decision.key)).toBe(true);expect(resolvePrimaryIncidentAction(reviewing,{public_id:"me",permissions:[]})).toBeNull();expect(resolvePrimaryIncidentAction(reviewing,{public_id:"other",permissions:["INCIDENT.DECIDE"]})).toBeNull();const completed={...incident,status:"ACTION_COMPLETED"} as DashboardIncident;expect(resolvePrimaryIncidentAction(completed,{public_id:"me",permissions:["INCIDENT.CLOSE"]})?.key).toBe("close");expect(resolvePrimaryIncidentAction(completed,{public_id:"other",permissions:["INCIDENT.CLOSE"]})).toBeNull();expect(resolvePrimaryIncidentAction(completed,{public_id:"other",permissions:["INCIDENT.CLOSE"],roles:["CONTROL_MANAGER"]})?.key).toBe("close");expect(isIncidentActionSupported("api","close")).toBe(true)});
