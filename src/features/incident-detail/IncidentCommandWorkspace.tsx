...
import { buildIncidentTimeline, canCompareEvidence, dedupeEvidences, isIncidentActionSupported, normalizeMemoType, reasonLabel, resolveIncidentWorkspaceMode, resolveMemoAvailability, resolvePrimaryIncidentAction, sortIncidentMemos } from "./incidentDetailDomain";
...
export function IncidentMemoLog({memos,userPublicId,canManage=false,onEdit,onDelete}:{memos:IncidentMemo[];userPublicId?:string;canManage?:boolean;onEdit?:(memo:IncidentMemo)=>void;onDelete?:(memo:IncidentMemo)=>void}){
...
export function WorkStageNavigator({record,selected,onSelect,onReturn}:{record:IncidentDetailRecord;selected:IncidentWorkStage|null;onSelect:(stage:IncidentWorkStage)=>void;onReturn:()=>void}){
  ...
  const currentIndex=Math.max(0,stages.findIndex(stage=>stage===current));
  ...
  return <section className="work-stage-navigation" aria-label="사건 업무 단계">
    <header><h3>업무 진행</h3><b>현재 단계 {currentIndex+1} / {stages.length}</b></header>
    ...
    <div className="work-stage-current"><span>현재 단계</span><strong>{current?.actionLabel??"처리 완료"}</strong><p>{incidentWorkStageFlow(stages)}</p></div>
  </section>;
}
...
export function DetailTabs({record,user,currentTask,onMemoChanged,onRefresh,onNotify,memoCapabilities={write:adapter.supportsMemoWrite,mutation:adapter.supportsMemoMutation}}:{record:IncidentDetailRecord;user:AuthenticatedUser|null;currentTask?:ReactNode;onMemoChanged:(memo:IncidentMemo)=>void;onRefresh:()=>Promise<boolean>;onNotify:(message:string)=>void;memoCapabilities?:MemoCapabilities}){
  ...
  return <section className="detail-tabs incident-workspace"><header className="incident-workspace__heading"><span>INCIDENT WORKSPACE</span><h2>사건 업무</h2></header><div className="detail-tabs__bar"><div className="detail-tabs__list" role="tablist" aria-label="사건 업무">{tabs.map((tab, index) => { const selected = active === tab.key; return <button id={`incident-workspace-tab-${tab.key}`} key={tab.key} ref={node => { tabRefs.current[index] = node; }} role="tab" tabIndex={selected ? 0 : -1} aria-selected={selected} aria-controls={`incident-workspace-panel-${tab.key}`} onClick={() => {setActive(tab.key);setSelectedStage(null)}} onKeyDown={event => { if (event.key === "ArrowRight") { event.preventDefault(); setSelectedStage(null);move(index, 1); } if (event.key === "ArrowLeft") { event.preventDefault();setSelectedStage(null); move(index, -1); } }}>{tab.label}{tab.count!==null&&<b aria-label={`${tab.count}건`}>{tab.count}</b>}</button>; })}</div></div>
    <div id={`incident-workspace-panel-${active}`} className="detail-tabs__panel incident-workspace__content" role="tabpanel" tabIndex={0} aria-labelledby={`incident-workspace-tab-${active}`}>
      ...
      {active === "history" && <div className="workspace-history"><div className="workspace-history-toolbar"><nav className="workspace-history-filters" aria-label="전체 기록 유형 필터">{(["ALL","사건","메모","출동"] as const).map(filter=><button type="button" key={filter} aria-pressed={historyFilter===filter} onClick={()=>setHistoryFilter(filter)}>{filter==="ALL"?"전체":filter}</button>)}</nav><span>정렬 <b>최신순</b></span></div>{historyFilter==="메모"?<IncidentMemoLog memos={record.memos} userPublicId={user?.publicId} canManage={memoCapabilities.mutation&&Boolean(user?.apiPermissions.includes("INCIDENT.DECIDE"))} onEdit={openEditMemo} onDelete={memo=>{setDeletingMemo(memo);setDeleteReason("");setMemoError("")}}/>:<>{record.dispatch&&historyFilter!=="사건"&&<dl className="workspace-dispatch-summary"><div><dt>출동 담당자</dt><dd>{record.dispatch.responder_label}</dd></div><div><dt>현재 출동 상태</dt><dd>{dispatchStatusLabel[record.dispatch.status]}</dd></div></dl>}{visibleTimeline.length?<ol className="activity-list" aria-label="최신순 사건 업무 기록">{visibleTimeline.map(item => <li key={item.id} className={item.id===latestTimelineId?"is-latest":""} data-type={item.type}><i aria-hidden="true"/><div><header><strong>{item.title}</strong><b data-type={item.type}>{item.type}</b></header><span>{item.actor} · <time dateTime={item.occurredAt}>{formatKst(item.occurredAt)} KST</time></span><TimelineDetail id={item.id.replaceAll(":","-")} type={item.type} detail={item.detail}/></div></li>)}</ol>:<p className="drawer-empty">{timeline.length?"선택한 유형의 기록이 없습니다.":"아직 표시할 업무 기록이 없습니다."}</p>}</>}</div>}
    </div>
      ...
      <div className="console-layout">
        ...
        <aside className="incident-workspace-column">
          ...
          <section className="command-task-status" aria-label="담당 상태"><h3>담당 상태</h3><div className="command-badges"><b>{incidentStatusLabel[incident.status]}</b><b>{incident.assigned_controller?.public_id===user?.publicId?"내 담당":incident.assigned_controller?"담당 지정":"담당 미지정"}</b></div></section>
           {dialog?<ActionDialog type={dialog} record={record} evidence={evidence} busy={busy} embedded onClose={() => setDialog(null)} onConfirm={payload => void perform(dialog === "decision" ? "decide" : dialog === "dispatch" ? "assign" : "close", payload)} />:record.decision&&mode==="READ_ONLY"?<FinalDecisionSummary record={record}/>:<><div className="command-next"><span>{awaitingController?"담당 관제자":"관제자가 할 일"}</span><strong>{awaitingController?"담당자 미지정":primary?.label ?? (mode === "READ_ONLY" ? "처리 완료" : "현재 가능한 업무 없음")}</strong><p>{awaitingController?"아직 이 사건을 담당하는 관제자가 없습니다.":primary ? actionDescription[primary.key] : mode === "READ_ONLY" ? "최종 처리된 사건입니다." : "현재 계정에는 이 단계의 사건 처리 권한이 없거나 아직 지원되지 않는 업무입니다."}</p></div>{claimButtonSuccess?<button className="incident-primary is-assignment-success" type="button" disabled><span aria-hidden="true">✓</span>내 담당으로 지정됨</button>:primary && <button className="incident-primary" disabled={busy} aria-busy={busy} onClick={() => primary.key === "decide" ? setDialog("decision") : primary.key === "assign" ? setDialog("dispatch") : primary.key === "close" ? setDialog("close") : void perform(primary.key as IncidentCommandAction)}>{busy ? primary.key === "claim" ? "담당 지정 중…" : "처리 중" : awaitingController?"내게 배정":primary.label}</button>}<p className="incident-permission-note">{primary?.key === "acknowledge" || primary?.key === "claim" ? "사건 확인 및 선점 권한이 필요합니다." : primary?.key === "review" || primary?.key === "decide" ? "사건 검토 권한이 필요합니다." : primary?.key === "assign" ? "출동 배정 권한이 필요합니다." : "권한과 사건 상태에 따라 업무가 제공됩니다."}</p>{incident.status === "CLAIMED" && adapter.mode === "mock" && <button className="incident-secondary" disabled={!adapter.supportsRelease} title={adapter.supportsRelease ? "담당 해제" : "준비 중"} onClick={() => adapter.supportsRelease && void perform("release")}>{adapter.supportsRelease ? "담당 해제" : "담당 해제 · 준비 중"}</button>}</>}
...
function ActionDialog({ type, record, evidence, busy, embedded=false, onClose, onConfirm }: { type: "decision" | "dispatch" | "close"; record:IncidentDetailRecord; evidence?:IncidentEvidence; busy: boolean; embedded?:boolean; onClose: () => void; onConfirm: (payload?: IncidentDecisionPayload | { responder_public_id: string; request_message: string }) => void }) {
  ...
  const content=<section className={`incident-dialog ${embedded?"incident-dialog--rail":""}`} role={embedded?"region":"dialog"} aria-modal={embedded?undefined:true} aria-labelledby="incident-dialog-title"><h2 id="incident-dialog-title">{type === "dispatch" ? "출동 담당자 배정" : "사건 종료 확인"}</h2><p>{type === "dispatch" ? "배정할 출동 담당자를 선택해 주세요." : "현장 조치 결과를 확인하고 사건을 종료합니다."}</p>{type === "dispatch" && <><label>출동 담당자<select value={responderId} disabled={!adapter.supportsDispatchAssignment || respondersLoading || !!respondersError} onChange={event => setResponderId(event.target.value)}><option value="">{respondersLoading ? "담당자를 불러오는 중입니다" : adapter.supportsDispatchAssignment ? "담당자를 선택하세요" : "출동 배정 준비 중"}</option>{responders.map(item => <option key={item.public_id} value={item.public_id} disabled={!item.available}>{item.display_name}{item.organization_name ? ` · ${item.organization_name}` : ""}{item.available ? "" : " · 배정 불가"}</option>)}</select></label>{respondersError && <p role="alert">{respondersError}</p>}<label>관제 요청 메시지<textarea value={message} disabled={!adapter.supportsDispatchAssignment} onChange={event => setMessage(event.target.value)} /></label></>}<div><button onClick={onClose}>취소</button><button disabled={busy || invalid} onClick={() => onConfirm(type === "dispatch" ? { responder_public_id: responderId, request_message: message.trim() } : undefined)}>{busy ? "처리 중" : "확인"}</button></div></section>;
