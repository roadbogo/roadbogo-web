...
function DetailTabs({ record,user,onMemoCreated,onRefresh,onNotify }: { record: IncidentDetailRecord;user:AuthenticatedUser|null;onMemoCreated:(memo:IncidentMemo)=>void;onRefresh:()=>Promise<boolean>;onNotify:(message:string)=>void }) {
  const tabs = useMemo(() => [
    ...
    adapter.supportsMemoRead ? { key: "memo", label: "관제 메모", count: record.memos.length } : null,
    { key: "dispatch", label: "출동 정보", count: record.dispatch ? 1 : 0 },
  ].filter(Boolean) as {key:string;label:string;count:number}[], [record.dispatch,record.histories.length,record.memos.length]);
  ...
  const availability=adapter.supportsMemoWrite?policyAvailability:{allowed:false,reason:"저장된 관제 메모는 현재 사건 기록에서 확인할 수 없습니다"};
  const memoTypes=availableMemoTypes(record.incident);
  ...
  const sortedHistories=useMemo(()=>sortIncidentHistories(record.histories),[record.histories]);
  const latestHistoryId = sortedHistories.at(-1)?.public_id;
  ...
  return <section className={`detail-tabs ${open?"is-open":""}`}><div className="detail-tabs__bar"><div className="detail-tabs__list" role="tablist" aria-label="사건 기록">{tabs.map((tab, index) => <button key={tab.key} ref={node => { tabRefs.current[index] = node; }} role="tab" aria-selected={active === tab.key} onClick={() => {setActive(tab.key);setOpen(true)}} onKeyDown={event => { if (event.key === "ArrowRight") move(index, 1); if (event.key === "ArrowLeft") move(index, -1); }}>{tab.label}{tab.count!==null&&<b aria-label={`${tab.count}건`}>{tab.count}</b>}</button>)}</div><button className="detail-tabs__toggle" type="button" aria-expanded={open} aria-controls="incident-record-drawer" onClick={()=>setOpen(value=>!value)}>{open?"사건 기록 닫기":"사건 기록 열기"}</button></div>
    <div id="incident-record-drawer" className="detail-tabs__panel" role="tabpanel" hidden={!open}>
      {active === "history" && <div className="activity-list">{sortedHistories.map(item => <article key={item.public_id} className={item.public_id === latestHistoryId ? "is-latest" : ""}><i /><div><strong>{item.label}</strong><span>{item.actor_name ?? "시스템"} · <time>{formatKst(item.occurred_at)} KST</time></span><p>{item.detail ?? "사건 상태가 갱신되었습니다."}</p></div></article>)}</div>}
...
export function IncidentCommandWorkspace({ publicId, invalidIdentifier = false }: { publicId: string; invalidIdentifier?: boolean }) {
  ...
  const perform = async (action: IncidentCommandAction, payload?: IncidentDecisionPayload | { responder_public_id: string; request_message: string }) => {
    ...
    try {
      ...
      if (result.ok) { setRecord(current => current ? { ...current, incident: { ...current.incident, status: result.status, version_no: result.version_no } } : current); const latest = result.record ?? await adapter.get(record.incident.public_id); if (latest) applyRecord(latest); const successMessage:Partial<Record<IncidentCommandAction,string>>={acknowledge:"사건을 확인했습니다.",claim:"이 사건의 담당자로 지정되었습니다.",release:"사건 담당이 해제되었습니다.",review:"사건 검토를 시작했습니다.",decide:"위험 판정을 반영했습니다.",assign:"출동 담당자를 배정했습니다.",close:"사건 종료를 확인했습니다."}; setToast(successMessage[action]??"사건 처리를 완료했습니다."); setDialog(null); }
