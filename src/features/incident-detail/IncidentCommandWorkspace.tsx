    ...
    try {
      ...
      if (result.ok) {
        ...
        if(action==="close"){
          ...
          const refreshed=await refreshIncidentAfterClosure(()=>adapter.get(record.incident.public_id),result.record);if(refreshed.record)applyRecord(refreshed.record);setSyncMessage(refreshed.warning);
        }else{
          const latest = result.record ?? await adapter.get(record.incident.public_id); if (latest) applyRecord(latest);
          const claimConfirmed=action==="claim"&&latest?.incident.status==="CLAIMED"&&latest.incident.assigned_controller?.public_id===user?.publicId;if(claimConfirmed){setToast("");setClaimNotice(true);setClaimButtonSuccess(true)}else{if(action==="release"){setClaimNotice(false);setClaimButtonSuccess(false)}setToastSuccess(true);setToast(successMessage[action]??"사건 처리를 완료했습니다.")} setDialog(null);
        }
      }
      else { if(result.latest){applyRecord(result.latest);if(action==="close"&&result.latest.incident.status==="CLOSED")setDialog(null)} const messages: Record<string, string> = { INCIDENT_VERSION_CONFLICT: "사건 정보가 변경되었습니다. 최신 상태를 다시 확인해 주세요.", INCIDENT_ALREADY_CLAIMED: `다른 관제자가 먼저 담당자로 지정되었습니다.${result.controller_name ? ` 담당자: ${result.controller_name}` : ""}`, INCIDENT_NOT_ASSIGNED_CONTROLLER: "현재 담당 관제자만 사건 검토를 시작할 수 있습니다.", INCIDENT_INVALID_STATE_TRANSITION: "현재 사건 상태에서는 이 작업을 수행할 수 없습니다.", DISPATCH_RESPONDER_NOT_FOUND: "출동 담당자를 찾을 수 없습니다.", DISPATCH_RESPONDER_UNAVAILABLE: "선택한 담당자는 현재 출동할 수 없습니다.", DISPATCH_RESPONDER_BUSY: "선택한 담당자가 다른 출동을 진행 중입니다.", DISPATCH_ALREADY_ASSIGNED: "이 사건에는 이미 활성 출동이 있습니다.", DISPATCH_IDEMPOTENCY_CONFLICT: "동일 멱등 키로 다른 출동 요청이 확인되었습니다.", INCIDENT_CLAIM_CONFLICT: "다른 관제자가 먼저 담당자로 지정되었습니다.", FORBIDDEN: "현재 권한으로 이 작업을 수행할 수 없습니다.", INVALID_TRANSITION: "현재 사건 상태에서 이 작업을 수행할 수 없습니다." }; setToastSuccess(false);setToast(messages[result.code] ?? "요청을 처리하지 못했습니다.");if(action==="close"&&result.sync_failed)setSyncMessage(closureConflictRefreshWarning); }
