...
function withStatus(base:DashboardIncident,status:IncidentStatus):DashboardIncident{const sequence=Object.keys(statusOrder).indexOf(status)+21;const assigned=["CLAIMED","UNDER_REVIEW","DISPATCH_REQUESTED","DISPATCHED","ON_SCENE","ACTION_IN_PROGRESS","ACTION_COMPLETED","CLOSED"].includes(status);return{...base,public_id:`22222222-2222-4222-8222-${String(sequence).padStart(12,"0")}`,incident_no:`INC-20260719-${String(sequence).padStart(4,"0")}`,status,version_no:3,assigned_controller:assigned?{public_id:"mock:controller@roadbogo.kr",display_name:"관제 담당자"}:null,claimed_at:assigned?new Date(Date.parse(base.created_at)+30*60*1000).toISOString():null}}
