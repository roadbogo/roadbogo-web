...
const permission:Partial<Record<IncidentStatus,string>>={NEW:"INCIDENT.CLAIM",ACKNOWLEDGED:"INCIDENT.CLAIM",CLAIMED:"INCIDENT.DECIDE",UNDER_REVIEW:"INCIDENT.DECIDE",DISPATCH_REQUESTED:"DISPATCH.ASSIGN",ACTION_COMPLETED:"INCIDENT.CLOSE"};
...
export function resolvePrimaryIncidentAction(incident:DashboardIncident,user:{public_id:string;permissions:string[]}){
  ...
  if(!Number.isInteger(incident.version_no)||incident.version_no<0)return null;return candidate;
}
