...
export function fieldActionClosureBlockReason(record:IncidentDetailRecord){
  if(!record.field_action)return "현장 조치 결과가 아직 연결되지 않았습니다.";
  ...
  if(!record.field_action.before_image_url||!record.field_action.after_image_url)return "조치 전·후 사진 연결이 완료되어야 사건을 종료할 수 있습니다.";
  if(!record.dispatch?.responder_label)return "출동 담당자 정보를 확인할 수 없습니다.";
  return null;
}

export function FieldActionReview({record,onClose,busy=false,closeButtonRef}:{record:IncidentDetailRecord;onClose?:()=>void;busy?:boolean;closeButtonRef?:Ref<HTMLButtonElement>}){
  const[preview,setPreview]=useState<Photo|null>(null),triggerRef=useRef<HTMLButtonElement|null>(null);
  const action=record.field_action,closed=record.incident.status==="CLOSED",blockReason=fieldActionClosureBlockReason(record);
  ...
  return <section className="field-action-review" aria-labelledby="field-action-title" data-closed={closed||undefined}>
    ...
    {action?<><dl><div><dt>출동 담당자</dt><dd>{record.dispatch?.responder_label??"정보 없음"}</dd></div><div><dt>조치 등록 시각</dt><dd>{action.completed_at?`${formatKst(action.completed_at)} KST`:"정보 없음"}</dd></div><div className="field-action-detail"><dt>조치 내용</dt><dd>{action.detail||"정보 없음"}</dd></div>{closed&&<div><dt>사건 종료 시각</dt><dd>{closeHistory?`${formatKst(closeHistory.occurred_at)} KST`:`${formatKst(record.incident.updated_at)} KST`}</dd></div>}</dl><div className="field-action-photos">{photos.map(photo=><article key={photo.label}><strong>{photo.label} 사진</strong><button type="button" onClick={event=>{triggerRef.current=event.currentTarget;setPreview(photo)}} aria-label={`${photo.label} 사진 확대 보기`}><img src={photo.url} alt={`${photo.label} 현장 사진`}/><span>확대 보기</span></button></article>)}</div></>:<p className="field-action-empty">현장 조치 결과가 아직 연결되지 않았습니다.</p>}
    {blockReason&&!closed&&<p className="field-action-block" role="alert"><strong>사건 종료 불가</strong>{blockReason}</p>}
    {!closed&&<button ref={closeButtonRef} className="field-action-close" type="button" disabled={Boolean(blockReason)||busy} onClick={onClose}>{busy?"처리 중":"사건 최종 종료"}</button>}
