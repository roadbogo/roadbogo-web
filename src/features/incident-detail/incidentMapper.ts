...
export function mapIncidentHistory(dto:IncidentHistoryDto):IncidentHistory{return{public_id:dto.public_id,event_type:"INCIDENT_UPDATED",label:incidentStatusLabel[dto.to_status],actor_name:dto.actor?.user_name??null,occurred_at:dto.changed_at,detail:dto.reason_text??dto.reason_code}}
export function mapIncidentDetailRecord(detail:IncidentDetailDto,evidences:IncidentEvidenceDto[],histories:IncidentHistoryDto[]):IncidentDetailRecord{return{incident:mapIncidentDetail(detail),cctv:mapIncidentCctv(detail),evidences:evidences.map((item,index)=>mapIncidentEvidence(item,index,detail)),histories:histories.map(mapIncidentHistory),dispatch:detail.active_dispatch?{public_id:detail.active_dispatch.public_id,incident_public_id:detail.public_id,status:detail.active_dispatch.status,responder_label:detail.active_dispatch.responder.user_name,requested_at:detail.active_dispatch.requested_at,updated_at:detail.active_dispatch.updated_at}:null,field_action:null,field_action_supported:false,decision:detail.decision?{result:detail.decision.decision_type,reason:detail.decision.decision_reason,decided_by:detail.decision.decided_by.user_name,decided_at:detail.decision.decided_at}:null,controller_note:null,memos:[],request_message:null,closed_at:detail.timeline.closed_at}}
export function mapDispatchResponder(dto:ResponderListItemDto):DispatchResponderOption{return{public_id:dto.public_id,display_name:dto.user_name,organization_name:dto.organization?.organization_name??null,available:dto.duty_status==="AVAILABLE"}}
