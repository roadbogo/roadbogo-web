...
export class MockDispatchAdapter implements DispatchAdapter {
  ...
  private uploadedFiles=new Map<string,{previewUrl:string;fileName:string}>();
  ...
  async uploadActionFile(file:File,purpose:DispatchActionFilePurpose,clientFileId:string,key:string){void key;const filePublicId=`mock-file-${clientFileId}`,previewUrl=purpose==="ACTION_BEFORE"?"/images/incidents/fallen-object-realistic.png":"/images/incidents/highway-traffic-realistic.png";this.uploadedFiles.set(filePublicId,{previewUrl,fileName:file.name});return{filePublicId}}
  async linkActionFile(publicId:string,filePublicId:string,purpose:DispatchActionFilePurpose,displayOrder:number,key:string){void key;const report=this.reports.get(publicId);if(!report)throw new Error("DISPATCH_ACTION_REPORT_REQUIRED");const uploaded=this.uploadedFiles.get(filePublicId),linked={filePublicId,purpose,displayOrder,fileName:uploaded?.fileName??filePublicId,previewUrl:uploaded?.previewUrl??null};this.reports.set(publicId,{...report,files:[...report.files.filter(file=>file.filePublicId!==filePublicId),linked]});return linked}
