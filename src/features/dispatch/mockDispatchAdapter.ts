...
export class MockDispatchAdapter implements DispatchAdapter {
  ...
  async uploadActionFile(file:File,_purpose:DispatchActionFilePurpose,clientFileId:string,key:string){void key;const filePublicId=`mock-file-${clientFileId}`;this.uploadedFiles.set(filePublicId,{previewUrl:URL.createObjectURL?.(file)??"",fileName:file.name});return{filePublicId}}
