...
describe("MockDispatchAdapter", () => {
  ...
  it("stores an action report, links photos, and completes the active dispatch",async()=>{
    ...
    const uploaded=await adapter.uploadActionFile(new File(["image"],"after.jpg"),"ACTION_AFTER","client-1","upload");const linked=await adapter.linkActionFile(requested.publicId,uploaded.filePublicId,"ACTION_AFTER",0,"link");expect(linked.previewUrl).toBe("/images/incidents/highway-traffic-realistic.png");expect((await adapter.getActionReport(requested.publicId))?.files).toHaveLength(1);
