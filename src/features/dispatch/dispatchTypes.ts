...
export type DispatchCommandResult={ok:true;detail:DispatchDetail;syncWarning?:"DETAIL_REFRESH_FAILED"}|{ok:false;code:string;latest:DispatchDetail|null};
export interface DispatchAdapter { readonly mode:"api"|"mock";list(query?:DispatchListQuery):Promise<DispatchPage>;detail(publicId:string):Promise<DispatchDetail|null>;accept(publicId:string,versionNo:number,idempotencyKey:string,current?:DispatchDetail):Promise<DispatchCommandResult>;reject(publicId:string,versionNo:number,reason:string,idempotencyKey:string,current?:DispatchDetail):Promise<DispatchCommandResult> }
