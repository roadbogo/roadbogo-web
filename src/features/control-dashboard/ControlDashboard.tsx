...
export function ControlDashboard(){
  ...
  const[selectedIncident,setSelectedIncident]=useState<string|null>(mockIncidentPublicIds["INC-20260719-0013"]); const[standaloneCctv,setStandaloneCctv]=useState<string|null>(null);
  ...
  const canAct=incident&&user?canUsePrimaryAction(incident,{publicId:user.publicId??"",apiPermissions:permissions}):false;
  ...
  return <div className="command-page"><LandingHeader showSections={false}/><main className="command-shell">
    ...
    <div className="command-workspace">
      <section className="command-wall"><header><div><span>LIVE CCTV WALL</span><h2>실시간 CCTV</h2></div><p>{data.cctvs.length}개 표시 · 정상 {normal} · 지연 {data.cctvs.filter(i=>i.operational_status==="DELAYED").length} · 장애 {data.cctvs.filter(i=>i.operational_status==="FAULT").length}</p></header>
        ...
        <div className="command-cctv-grid">{data.cctvs.map(item=>{const linked=prioritizeIncidents(activeIncidents.filter(i=>i.cctv_public_id===item.public_id))[0];const selected=item.public_id===selectedCctv;return <article key={item.public_id} className={`command-cctv ${selected?"is-selected":""}`} aria-current={selected?"true":undefined} onClick={()=>chooseCctv(item)} onDoubleClick={event=>{if(item.has_stream){setFocusSource(event.currentTarget);setFocusCctv(item.public_id)}}} tabIndex={0} onKeyDown={event=>{if(event.key==="Enter"||event.key===" "){event.preventDefault();chooseCctv(item)}}}><div className="command-cctv__top"><div><strong>{item.cctv_name}</strong><span className={`device-${item.operational_status.toLowerCase()}`}>{operationalStatusLabel[item.operational_status]}</span>{selected&&<em>선택됨</em>}</div><span>{item.source_type} · {item.stream_type}</span></div><div className="command-cctv__viewport"><Video cctv={item} incident={linked}/><button className="command-cctv__focus" type="button" disabled={!item.has_stream} aria-label={`${item.cctv_name} 집중 관제 열기`} onClick={event=>{event.stopPropagation();setFocusSource(event.currentTarget);setFocusCctv(item.public_id)}}><ExpandIcon/></button></div><footer><div><strong>{item.road.road_name}</strong><span>{item.road_section.section_name} · {directionLabel[item.direction_code]}</span></div></footer>{linked?<div className="command-cctv__incident"><span>{linked.incident_no}</span><b>{incidentStatusLabel[linked.status]}</b></div>:<div className="command-cctv__incident is-empty"><span>진행 사건 없음</span><b>대기</b></div>}</article>})}</div>
      </section>
