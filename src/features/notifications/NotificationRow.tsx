  ...
  return <article className={`${styles.row} ${!item.read ? styles.unread : ""} ${compact ? styles.compactRow : ""}`}>
    <button type="button" className={styles.rowMain} onClick={() => onOpen(item)} aria-label={`${item.read ? "읽음" : "읽지 않음"} · ${item.title}`}>
      ...
      <span className={styles.rowCopy}>
        ...
        {showOperationsMetadata && <span className={styles.rowMeta}><b className={styles[`severity${item.severity}`]}>{severityLabels[item.severity]}</b><span>{item.resource_label}</span><span>{notificationStateCopy(item)}</span></span>}
      </span>{compact && <span className={styles.rowChevron} aria-hidden="true"><svg viewBox="0 0 24 24"><path d="m9 6 6 6-6 6" /></svg></span>}
    </button>
    {showOperationsMetadata && !compact && item.action_required && item.action_label && item.target_path && <button type="button" className={styles.rowAction} onClick={() => onOpen(item)}>{item.action_label}</button>}
  </article>;
}
