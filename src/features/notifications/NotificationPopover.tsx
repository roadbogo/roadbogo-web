...
export function NotificationPopover() {
  ...
  return <div ref={rootRef} className={styles.popoverRoot}>
    ...
    {open && <><button type="button" className={styles.mobileBackdrop} aria-label="업무 알림 닫기" onClick={close} />
      <section ref={panelRef} id="notification-popover" className={styles.popover} role="dialog" aria-modal="false" aria-labelledby="notification-popover-title">
        <header>
          <div><h2 id="notification-popover-title">업무 알림</h2><p>{tab === "action" ? `처리가 필요한 업무 ${actionCount}건` : `읽지 않은 알림 ${unreadCount}건`}</p></div>
          <Link href="/notifications" onClick={() => setOpen(false)}>전체 보기</Link>
          <button type="button" className={styles.mobileClose} aria-label="업무 알림 닫기" onClick={close}><CloseIcon /></button>
        </header>
        <div className={styles.popoverTabs} role="tablist" aria-label="업무 알림 분류">
        ...
        <div className={styles.popoverScroll} id={`notification-popover-panel-${tab}`} role="tabpanel" aria-labelledby={`notification-popover-tab-${tab}`}>
          {loading ? <div className={styles.skeleton} aria-label="알림을 불러오는 중"><i /><i /><i /></div>
            : error ? <div className={styles.popoverEmpty}><strong>알림을 불러오지 못했습니다</strong><span>{error}</span></div>
              : items.length === 0 ? <div className={styles.popoverEmpty}><strong>새로운 업무 알림이 없습니다</strong><span>새로운 안내가 도착하면 이곳에서 확인할 수 있습니다</span></div>
        ...
        <footer><i aria-hidden="true" /><span>{realtimeStatus === "connecting" ? "연결 중" : realtimeStatus === "recovering" ? "연결 복구 중" : "데모 알림 표시 중"}</span></footer>
      </section>
    </>}
  </div>;
}
