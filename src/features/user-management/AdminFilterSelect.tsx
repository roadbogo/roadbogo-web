...
export function AdminFilterSelect<T extends string>({label,value,options,onChange,triggerIcon,applied=value!==options[0]?.value}:{label:string;value:T;options:AdminFilterOption<T>[];onChange:(value:T)=>void;triggerIcon?:ReactNode;applied?:boolean}){
 ...
 const popup=open&&typeof document!=="undefined"?createPortal(<><button className={styles.backdrop} tabIndex={-1} aria-label={`${label} 선택 닫기`} onClick={()=>setOpen(false)}/><section ref={menu} id={`${id}-menu`} className={styles.menu} style={position} role="listbox" aria-label={`${label} 필터`} onKeyDown={key}><header><strong>{label} 선택</strong><button type="button" aria-label={`${label} 선택 닫기`} onClick={()=>{setOpen(false);trigger.current?.focus()}}>×</button></header><div>{options.map((option,index)=><button id={`${id}-${index}`} type="button" role="option" aria-selected={option.value===value} data-active={active===index} data-icon={Boolean(option.icon)} title={option.title} key={option.value} onMouseEnter={()=>setActive(index)} onClick={()=>choose(option.value)} style={{"--option-accent":option.accent??"#53666f","--option-surface":option.surface??"#eef2f2","--option-border":option.border??"#d8e1e3"} as CSSProperties}>{option.icon&&<span className={styles.icon}>{option.icon}</span>}<span>{option.label}</span>{option.count!==undefined&&<small>{option.count}명</small>}<span className={styles.check} aria-hidden="true">{option.value===value?"✓":""}</span></button>)}</div></section></>,document.body):null;
 return <div className={styles.root}><button ref={trigger} type="button" className={styles.trigger} data-detailed={Boolean(triggerIcon)} data-applied={applied} aria-haspopup="listbox" aria-expanded={open} aria-controls={`${id}-menu`} aria-label={`${label} 필터, 현재 ${selected?.label}`} title={selected?.label} onClick={()=>setOpen(current=>!current)} onKeyDown={key}>{triggerIcon?<><span className={styles.triggerIcon}>{triggerIcon}</span><span className={styles.triggerText}><small>{label}</small><strong>{selected?.label}</strong></span><i className={styles.selectorIcon} aria-hidden="true"><svg viewBox="0 0 24 24"><path d="m8 9 4-4 4 4M16 15l-4 4-4-4"/></svg></i></>:<><span>{label} · {selected?.label}</span><i aria-hidden="true">⌄</i></>}</button>{popup}</div>;
}
