  ...
  return <main className={styles.page}>
    ...
    <div className={`${styles.workspace}${selectedId?` ${styles.withInspector}`:""}${deactivationOpen?` ${styles.deactivationMode}`:""}`}>
      <section className={styles.listPanel} ref={listTop} aria-labelledby="user-list-title" aria-busy={listLoading}>
        ...
        {listError?<div className={styles.state} role="alert"><strong>Mock 사용자 목록을 불러오지 못했습니다.</strong><button type="button" onClick={()=>void loadList()}>다시 시도</button></div>:!result?<div className={styles.skeleton}>{Array.from({length:8},(_,index)=><i key={index}/>)}</div>:result.items.length===0?<div className={styles.state}><strong>{query.view==="attention"&&!hasConditions?"현재 검토가 필요한 사용자가 없습니다.":hasConditions?"조건에 맞는 사용자가 없습니다.":"등록된 사용자가 없습니다."}</strong><p>{hasConditions?"검색어나 필터 조건을 변경해 주세요.":"운영 사용자를 등록하면 이곳에서 관리할 수 있습니다."}</p>{hasConditions?<button type="button" onClick={clearAll}>조건 초기화</button>:canWrite?<Link href="/admin/users/new">운영 사용자 등록</Link>:null}</div>:<div role="listbox" aria-label="사용자 선택" className={`${styles.rows}${listLoading?` ${styles.isUpdating}`:""}`}>{result.items.map(item=>{const selected=item.publicId===selectedId,checks=userChecks(item),recovery=getAccountRecoveryEligibility(item),deactivation=item.changes.find(change=>change.action.includes("비활성"));return <button role="option" type="button" className={selected?styles.selected:undefined} aria-selected={selected} key={item.publicId} onClick={event=>choose(item,event.currentTarget)}><span className={styles.identity}><i aria-hidden="true">{item.userName.trim().charAt(0)}</i><span><strong title={item.userName}>{item.userName}</strong><small title={item.email}>{item.email}</small>{checks.length>0&&<small className={styles.reviewReason} title={checks.join(", ")}>{checks[0]}{checks.length>1?` +${checks.length-1}`:""}</small>}</span></span><span title={item.organization?.name}>{item.organization?.name??"소속 미지정"}</span><span className={styles.roleBadges}>{item.roles.length?<RoleBadgeGroup roles={item.roles} maxVisible={1}/>:<b className={styles.unassigned}>역할 미지정</b>}</span>{query.view==="inactive"?<><span><strong>{item.deactivatedAt?formatKst(item.deactivatedAt):"기록 확인 필요"}</strong><small>{deactivation?.summary??"기록 확인 필요"}</small></span><span><em className={recovery.eligibility==="AVAILABLE"?styles.active:styles.inactive}>{recovery.eligibility==="AVAILABLE"?"✓ 복구 가능":recovery.eligibility==="UNAVAILABLE"?"× 복구 불가":"! 확인 필요"}</em></span></>:<><span><em className={item.accountStatus==="ACTIVE"?styles.active:styles.inactive}>● {statusLabel[item.accountStatus]}</em></span><span title={item.lastLoginAt?`${formatKst(item.lastLoginAt)} KST`:"로그인 기록 없음"}>{item.lastLoginAt?formatKst(item.lastLoginAt):"기록 없음"}</span></>}</button>})}</div>}
