...
export function AccountActivationReview({user,adapter,styles,onClose,onSuccess}:{user:ManagedUser;adapter:UserManagementAdapter;styles:Styles;onClose:()=>void;onSuccess:(user:ManagedUser)=>void}){
  ...
  const recovery=getAccountRecoveryEligibility(user),available=recovery.eligibility==="AVAILABLE";
  ...
  return <div className={styles.activationReview} aria-busy={saving}>
    <header className={styles.inspectorHead}><div><span className={available?styles.recoveryAvailable:styles.recoveryReview}>{available?"✓ 복구 가능":recovery.eligibility==="UNAVAILABLE"?"× 복구 불가":"! 확인 필요"}</span><h2>비활성 계정 검토</h2><p>{user.userName} · {user.email}</p></div><button type="button" aria-label="비활성 계정 검토 닫기" onClick={close}>×</button></header>
    <div className={styles.activationBody}>
      <section><h3>사용자 요약</h3><strong>{user.userName}</strong><p>{user.email}</p><p>{recovery.reason}</p></section>
