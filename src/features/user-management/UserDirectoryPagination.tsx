...
export function UserPageSizeSelect({value,onChange,unit="명",compact=false}:{value:number;onChange:(value:number)=>void;unit?:"명"|"대"|"건";compact?:boolean}){
 ...
 const popup=open&&typeof document!=="undefined"?createPortal(<div ref={menu} className={`${styles.menu}${compact?` ${styles.compactMenu}`:""}`} style={position} role="listbox" aria-label={`한 페이지에 표시할 항목 수 (${unit})`} onKeyDown={key}>{USER_PAGE_SIZE_OPTIONS.map((option,index)=><button type="button" role="option" aria-selected={value===option} data-active={active===index} key={option} onMouseEnter={()=>setActive(index)} onClick={()=>choose(option)}>{!compact&&<span aria-hidden="true">{value===option?"✓":""}</span>}<b>{option}{unit}{compact?"씩":""}</b></button>)}</div>,document.body):null;
 return <span className={styles.root}>{!compact&&<span>한 페이지에</span>}<button ref={trigger} type="button" className={styles.trigger} aria-haspopup="listbox" aria-expanded={open} aria-label={`한 페이지에 ${value}${unit} 표시`} onClick={()=>setOpen(current=>!current)} onKeyDown={key}><b>{value}{unit}{compact?"씩":""}</b>{!compact&&<i aria-hidden="true">⌄</i>}</button>{popup}</span>;
}
