...
interface Props{incident:DashboardIncident|null;cctv:DashboardCctv|null;dispatch:DashboardDispatch|null;dispatchLookupStatus?:DispatchLookupStatus;canAct:boolean;blockedReason?:string|null;className?:string}
export function SelectedIncidentPanel({incident,cctv,dispatch,dispatchLookupStatus="ready",canAct,blockedReason,className=""}:Props){
 ...
 return <section className={`selected-incident-panel ${className}`} aria-label="선택 사건">
  ...
  <dl className="selected-incident-panel__assignment"><div><dt>담당 관제자</dt><dd>{dispatchView.controller}</dd></div><div><dt>출동 담당자</dt><dd>{dispatchUnknown?(dispatchLookupStatus==="loading"?"확인 중":"확인 실패"):dispatchView.responder}</dd></div>{dispatchView.hasDispatch&&<div><dt>출동 상태</dt><dd>{dispatchView.status}</dd></div>}</dl>
  <div className="selected-incident-panel__work"><span>다음 업무</span><strong>{displayedWork.title}</strong><p>{waitingForResponder||dispatchUnknown?displayedWork.description:blockedReason??displayedWork.description}</p>{displayedWork.button&&canAct&&!dispatchUnknown&&<Link className="command-primary" href={incidentDetailPath(incident.public_id)}>{displayedWork.button}</Link>}</div>
