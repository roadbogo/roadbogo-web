...
export function AuthIntentTabs({ intent, onChange }: Props) {
  ...
  return <div className={styles.tabs} data-login-tabs role="tablist" aria-label="접속 목적" onKeyDown={handleKeys}>
    <button data-intent="general" type="button" role="tab" aria-selected={intent === "general"} onClick={() => onChange("general")}>일반 서비스</button>
    <button data-intent="operations" type="button" role="tab" aria-selected={intent === "operations"} onClick={() => onChange("operations")}>운영 시스템</button>
