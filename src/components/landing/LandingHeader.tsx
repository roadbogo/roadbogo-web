...
export function LandingHeader({ showSections = true }: { showSections?: boolean } = {}) {
  ...
  return <>
    ...
    <header className="main-header"><div className="main-header__inner">
      ...
      {showPublicDrawer && <button ref={mobileTriggerRef} type="button" className="mobile-menu-trigger" aria-label={sidebarOpen?"메뉴 닫기":"메뉴 열기"} aria-expanded={sidebarOpen} aria-controls="landing-sidebar" onClick={() => sidebarOpen?close(true):setSidebarOpen(true)}><MenuIcon /></button>}
    </div></header>
