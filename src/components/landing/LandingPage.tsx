...
export function LandingPage(){
 ...
 return <div className="landing-page"><LandingHeader/><main>
  <section id="home" className="command-stage"><div className="stage-copy"><p className="stage-eyebrow"><span/> ROAD FLOW INTELLIGENCE</p><h1 className="stage-title"><span>도로 위 위험을</span><span className="stage-title__accent">AI가 먼저 발견하고,</span><span>대응까지 연결합니다</span></h1><p className="stage-description"><span>AI가 도로 위 위험 요소를 실시간으로 감지하고,</span><span>관제 판단부터 현장 대응까지 하나의 흐름으로 연결합니다.</span></p><div className="stage-actions">{showControlCta&&<Link href="/control" className="stage-primary">실시간 관제 보기 <span>→</span></Link>}<button className={showControlCta?undefined:"stage-primary"} type="button" onClick={showFlow}>운영 체계 안내 <span>↓</span></button></div></div><LandingCarousel/></section>
