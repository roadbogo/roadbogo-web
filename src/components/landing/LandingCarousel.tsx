...
const incidentData = [
  { id: "fallen-object", label: "낙하물", detectionLabel: "낙하물 감지", score: 92, confidence: [89, 91, 92], image: "/images/incidents/fallen-object-realistic.png", incident: "RB-260714-0821", position: "debris", bbox: { x: .455, y: .54, width: .14, height: .19 }, lane: "2차로 · 137.4K", objectStatus: "차로 내 정지" },
  { id: "wild-animal", label: "야생동물", detectionLabel: "야생동물 감지", score: 88, confidence: [85, 87, 88], image: "/images/incidents/wild-animal-realistic.png", incident: "RB-260714-0824", position: "wildlife", bbox: { x: .46, y: .51, width: .13, height: .22 }, lane: "1차로 · 137.4K", objectStatus: "도로 진입" },
  { id: "motorcycle", label: "이륜차", detectionLabel: "이륜차 감지", score: 85, confidence: [82, 84, 85], image: "/images/incidents/motorcycle-realistic.png", incident: "RB-260714-0827", position: "motorcycle", bbox: { x: .47, y: .46, width: .15, height: .25 }, lane: "갓길 인접 · 137.4K", objectStatus: "이동 중" },
] as const;
const vehicleDetections:{id:string;label:string;confidence:number;bbox:NormalizedBBox;labelPosition:"top-left"|"top-right"}[]=[
  {id:"vehicle-one",label:"vehicle",confidence:98,bbox:{x:.078,y:.04,width:.075,height:.1},labelPosition:"top-left"},
  {id:"vehicle-two",label:"vehicle",confidence:97,bbox:{x:.368,y:.09,width:.135,height:.17},labelPosition:"top-left"},
  {id:"vehicle-three",label:"vehicle",confidence:96,bbox:{x:.715,y:.365,width:.25,height:.32},labelPosition:"top-right"},
];
...
export function LandingCarousel() {
  ...
  return <section id="incident-stage" tabIndex={0} onKeyDown={(event) => { if (event.key === "ArrowLeft") move(-1); if (event.key === "ArrowRight") move(1); }} className={`incident-stage incident-stage--${activeScene}`} aria-label="라이브 사건 처리 시연">
    ...
    <div className="incident-body"><div className="cctv-frame"><div className="cctv-meta"><span>CAM 07 · 중부고속도로 137.4K · DEMO</span><span>2026.07.14 · {formatTime(currentTime)}</span></div><div className="road-scene"><DetectionOverlay key={transitionKey} objectType={scenario.id} variant="danger" label={scenario.detectionLabel} confidence={confidence} bbox={scenario.bbox} className={`detected-object detected-object--${scenario.position}`} compactLabel><Image src={scenario.image} alt={scenario.label} fill sizes="12vw" /></DetectionOverlay>{vehicleDetections.map((vehicle)=><DetectionOverlay key={vehicle.id} objectType="vehicle" variant="normal" label={vehicle.label} confidence={vehicle.confidence} bbox={vehicle.bbox} className={`vehicle-detection ${vehicle.id} vehicle-detection--label-${vehicle.labelPosition}`} compactLabel animated={false}/>) }<span className="camera-label">AI TRACKING DEMO · Objects 12 · Risk 01</span></div></div><article className="incident-card response-panel scenario-fade" key={`card-${scenario.id}`}>
